...
export function TaraPesajeDialog({
    ...
    const schema = useMemo(() => createTaraSchema(pesoBruto), [pesoBruto])
    ...
    const onSubmit: SubmitHandler<TaraSchema> = ({ tara }) => onConfirm(tara)

    const tara = form.watch('tara')
    const pesoNeto = pesoBruto - (tara ?? 0)
